---
title: 0xkholod
description: A living offensive security knowledge base focused on pentesting, red teaming, scripting, and practical tradecraft.
cssclasses:
  - card-list
  - purple-style
banner: "https://images.unsplash.com/photo-1501471984908-815b996862f4?q=80&w=2069&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
---

> ***A living offensive security knowledge base by [0xkholod](https://github.com/4l3xBB).***

This field manual turns research, lab work, and hands-on experience into concise, reusable notes. It is built for fast retrieval during study, assessments, and controlled security testing.

### *Core Collections* ⟡

- ![](https://media0.giphy.com/media/26xBQxJc5JzAtLx1C/giphy.gif?cid=6c09b952uz02zvkfe1bx3fxt82i5u7ul58fncsegw7tkzfrs&ep=v1_internal_gif_by_id&rid=giphy.gif&ct=g)
	- [[PENTESTING|PENTESTING]]
- ![](https://media0.giphy.com/media/26xBQxJc5JzAtLx1C/giphy.gif?cid=6c09b952uz02zvkfe1bx3fxt82i5u7ul58fncsegw7tkzfrs&ep=v1_internal_gif_by_id&rid=giphy.gif&ct=g)
	- [[SCRIPTING|SCRIPTING]]
- ![](https://media0.giphy.com/media/26xBQxJc5JzAtLx1C/giphy.gif?cid=6c09b952uz02zvkfe1bx3fxt82i5u7ul58fncsegw7tkzfrs&ep=v1_internal_gif_by_id&rid=giphy.gif&ct=g)
	- [[SETUP|LAB & SETUP]]

<br>

### *What You Will Find* ︎ ⌕

- Pentesting and red-team methodology
- Protocol, service, and Active Directory references
- Web exploitation notes and machine write-ups
- Scripts, command patterns, tooling, and lab setup

> [!warning]- *Responsible Use*
> This material is published for education, research, and authorized security testing only. Always obtain explicit permission before testing systems you do not own.

The manual is continuously refined as techniques, tools, and my own understanding evolve.
